...
export default function ContactPage() {
  ...
  const tr = (he: string, en: string, es: string) =>
    ...
    <div style={{ minHeight: "100vh", background: "#0a1628", color: "#ffffff", fontFamily: "'Rubik',sans-serif", direction: dir }}>
      ...
      <div style={{ maxWidth: 560, margin: "0 auto", padding: "60px 24px 80px" }}>
        ...
        <h1 style={{ fontSize: 32, fontWeight: 900, color: "#64dfdf", marginBottom: 12 }}>
          {tr("צור קשר", "Contact Us", "Contacto")}
        </h1>
        <p style={{ color: "rgba(255,255,255,0.55)", fontSize: 15, lineHeight: 1.8, marginBottom: 40 }}>
          {tr(
            ...
            "Nos encantaría saber de ti — preguntas, problemas, ideas o cualquier otra cosa."
          )}
        </p>
        ...
        <div style={{ background: "rgba(100,223,223,0.06)", border: "0.5px solid rgba(100,223,223,0.25)", borderRadius: 16, padding: "20px 24px", marginBottom: 24 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
            ...
            <button onClick={copy} style={{ padding: "7px 14px", borderRadius: 8, border: "0.5px solid rgba(100,223,223,0.3)", background: copied ? "rgba(100,223,223,0.2)" : "rgba(100,223,223,0.08)", color: "#64dfdf", fontWeight: 700, fontSize: 12, cursor: "pointer", fontFamily: "'Rubik',sans-serif", transition: "all 0.2s", whiteSpace: "nowrap" }}>
              {copied
                ? <><Check size={13} strokeWidth={2.5}/> {tr("הועתק", "Copied", "Copiado")}</>
                : <><Copy size={13} strokeWidth={1.5}/> {tr("העתק", "Copy", "Copiar")}</>}
            </button>
          </div>
        </div>
        ...
        <div style={{ background: "rgba(255,255,255,0.03)", border: "0.5px solid rgba(255,255,255,0.08)", borderRadius: 12, padding: "14px 18px", marginBottom: 40, display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <span style={{ color: "rgba(255,255,255,0.5)", fontSize: 13 }}>
            {tr("זמן תגובה ממוצע: עד 48 שעות", "Average response time: up to 48 hours", "Tiempo de respuesta promedio: hasta 48 horas")}
          </span>
        </div>

        {/* Topics */}
        <h2 style={{ fontSize: 15, fontWeight: 700, color: "rgba(255,255,255,0.7)", marginBottom: 14 }}>
          {tr("במה נוכל לעזור?", "How can we help?", "¿En qué podemos ayudarte?")}
        </h2>
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {[
            { Icon: Bug,       color: "#f472b6", bg: "rgba(244,114,182,0.12)", title: tr("דיווח על תקלה", "Bug report", "Reportar un error"),               desc: tr("משהו לא עובד כמו שצריך", "Something isn't working as expected", "Algo no funciona como debería") },
            { Icon: Lightbulb, color: "#fbbf24", bg: "rgba(251,191,36,0.12)",  title: tr("רעיון לפיצ'ר",   "Feature idea",            "Idea de función"),     desc: tr("יש לך רעיון שישפר את האפליקציה", "You have an idea to improve the app", "Tienes una idea para mejorar la app") },
            { Icon: Lock,      color: "#818cf8", bg: "rgba(129,140,248,0.12)", title: tr("פרטיות ומחיקת חשבון", "Privacy & account deletion", "Privacidad y eliminación de cuenta"), desc: tr("בקשה למחיקת נתוניך", "Request to delete your data", "Solicitud para eliminar tus datos") },
            { Icon: Users,     color: "#4ade80", bg: "rgba(74,222,128,0.12)",  title: tr("שיתוף פעולה",     "Partnership",             "Colaboración"),        desc: tr("עסקי, שיווקי או אחר", "Business, marketing, or other", "Negocios, marketing u otros") },
          ].map(item => (
